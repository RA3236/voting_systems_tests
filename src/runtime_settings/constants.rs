...
pub const DENSITY_NUM_BINS: usize = const {
    DENSITY_INT_RANGE[1] - DENSITY_INT_RANGE[0] + 1
};
/// The size of each density bin in one axis.
pub const DENSITY_STEP: f64 = const {
    (POPULACE_BOUNDS[1] - POPULACE_BOUNDS[0]) / ((DENSITY_NUM_BINS - 1) as f64)
};
...
pub static DENSITY_X_LABELS: LazyLock<[String; DENSITY_NUM_BINS]> = LazyLock::new(|| {
    DENSITY_X
        .map(|x| format!("{x:.2}"))
});
/// The Y labels for the density visualizations.
pub static DENSITY_Y_LABELS: LazyLock<[String; DENSITY_NUM_BINS]> = LazyLock::new(|| {
    DENSITY_Y
        .map(|y| format!("{y:.2}"))
});